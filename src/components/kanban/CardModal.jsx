...
export function CardModal({ card, colId, projectId, project, onClose, onUpdate, onMove, onDelete }) {
  ...
  return (
    <>
    {showPlanModal && (
      <PlanModal card={card} onClose={() => setShowPlanModal(false)} />
    )}
    <Modal onClose={onClose} width={520}>
      ...
      {editing ? (
        ...
        <div>
          ...
          {project?.claudeEnabled && colId === "todo" && (
            <PlanPanel card={card} projectId={projectId} onViewPlan={() => setShowPlanModal(true)} />
          )}
